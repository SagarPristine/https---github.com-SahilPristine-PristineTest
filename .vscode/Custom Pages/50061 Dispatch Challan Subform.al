page 50061 "Dispatch Challan Subform"
{
    AutoSplitKey = true;
    Caption = 'Lines';
    DelayedInsert = true;
        ...
        area(processing)
        {
            ...
            group("&Line")
            {
                ...
                group("F&unctions")
                {
                    ...
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Get Do Lines';
                        Ellipsis = true;
                        Image = GetLines;
                        trigger OnAction()
                        var
                            DeliveryOrderLinePage: Page "Delivery Sales Subform";
                            DeliverySalesLine: Record "Delivery Sales line";
                            SalesHdr: Record "Sales Header";
                            SelectFilter: Text;
                            SalesLine: Record "Sales Line";
                        begin
                            SalesHdr.get(Rec."Document Type", rec."Document No.");
                            DeliverySalesLine.Reset();
                            DeliverySalesLine.SetRange("Document No.", SalesHdr."Delivery Order No");
                            DeliveryOrderLinePage.LookupMode(true);
                            DeliveryOrderLinePage.SetTableView(DeliverySalesLine);
                            if DeliveryOrderLinePage.RunModal() = Action::LookupOK then
                                SelectFilter := DeliveryOrderLinePage.GetSelectionFilterDOLines;

                            if SelectFilter <> '' then begin
                                DeliverySalesLine.Reset();
                                DeliverySalesLine.SetCurrentKey("Line No.");
                                DeliverySalesLine.SetRange("Document No.", SalesHdr."Delivery Order No");
                                DeliverySalesLine.SetFilter("Line No.", SelectFilter);
                                if DeliverySalesLine.FindSet() then begin
                                    repeat
                                        SalesLine.Reset();
                                        SalesLine.Init();
                                        ...
                                        SalesLine."Line No." := DeliverySalesLine."Line No.";
                                        SalesLine.Validate("No.", DeliverySalesLine."Item No.");
                                        SalesLine.Validate("No. of Bags/Pckt", DeliverySalesLine."No. of bags");
                                        SalesLine.Validate(Quantity, DeliverySalesLine.Quantity);
                                        SalesLine.Insert();
                                    until DeliverySalesLine.Next() = 0;
                                end;
                            end;
