page 50094 "Seed Arrival"
{
    ...
    layout
    {
        area(content)
        {
            ...
            group(Logistics)
            {
                Caption = 'Logistic Details';

                field("Party DC/Bill No."; Rec."Party DC/Bill No.")
                {
                    ApplicationArea = All;
                    ToolTip = 'Specifies the value of the Party DC/Bill No. field.';
                }
                field("Party DC/Bill Date"; Rec."Party DC/Bill Date")
                {
                    ApplicationArea = All;
                    ToolTip = 'Specifies the value of the Party DC/Bill Date field.';
                }
            ...
            part(PurchLines; "Seed Arrival Subform")
            {
                ApplicationArea = Suite;
                Editable = "Buy-from Vendor No." <> '';
            ...
            group("Custom Process")
            {
                ...
                action(CreateLotNo)
                {
                    ...
                    var
                        ...
                        PlantingLtLn: Record "Planting List Line";
                        ReservationEntry: Record "Reservation Entry";
                        entryno: Integer;
                        lastLotIndicator: Text[100];
                    begin
                        Purchaseline.RESET;
                        Purchaseline.SETCURRENTKEY("Document No.");
                        Purchaseline.SETRANGE("Document No.", "No.");
                        Purchaseline.SetRange("Production Lot Status", false);
                        IF Purchaseline.FINDFIRST THEN BEGIN
                            repeat
                                PlantingLtLn.RESET;
                                PlantingLtLn.SETCURRENTKEY("Production Lot No.");
                                PlantingLtLn.SETRANGE("Production Lot No.", Purchaseline."Production Lot No.");
                                IF PlantingLtLn.FINDFIRST THEN BEGIN
                                    ReservationEntry.RESET;
                                    ReservationEntry.SETCURRENTKEY("Source ID", "Source Batch Name", "Source Type", "Created By", "Variant Code", "Item Tracking", "Source Ref. No.", "Reservation Status", "Location Code", Positive, "Item No.");
                                    ReservationEntry.SETRANGE("Source ID", Purchaseline."Document No.");
                                    ReservationEntry.SETRANGE("Source Type", 39);
                                    ReservationEntry.SETRANGE("Created By", USERID);
                                    ...
                                    if PlantingLtLn.Counter = '' then
                                        ReservationEntry."Lot No." := PlantingLtLn."Production Lot No." + '-' + FORMAT('A')
                                    else begin
                                        lastLotIndicator := PlantingLtLn.Counter;
                                        IF INCSTR(lastLotIndicator) = '' THEN BEGIN
                                            lastLotIndicator[STRLEN(lastLotIndicator)] := lastLotIndicator[STRLEN(lastLotIndicator)] + 1;
                                        END ELSE
                                            lastLotIndicator := INCSTR(lastLotIndicator);

                                        ReservationEntry."Lot No." := PlantingLtLn."Production Lot No." + '-' + FORMAT(lastLotIndicator);
                                    end;
                                    ...
                                    Purchaseline."Production Lot Status" := true;
                                    Purchaseline.MODIFY;
                                    if lastLotIndicator = '' then
                                        PlantingLtLn.Counter := 'A'
                                    else
                                        PlantingLtLn.Counter := lastLotIndicator;
                                    PlantingLtLn.MODIFY;
                                END;
                ...
                action(Calculatebardana)
                {
                    ApplicationArea = Suite;
                    Caption = 'Calculate Bardana';
                    Image = Calculate;
                    Promoted = true;
                    PromotedCategory = Process;
                    PromotedIsBig = true;
                    ...
                    var
                        ...
                        BardanaAdjmtMr: Record "Bardana Adjustment Master";
                        text1: Text;
                    begin
                        if Rec.PurchLinesExist() then begin
                            PurchLn.Reset();
                            PurchLn.SetCurrentKey("Document Type", "Document No.", "Line No.");
                            PurchLn.SetRange("Document Type", Rec."Document Type");
                            PurchLn.SetRange("Document No.", Rec."No.");
                            PurchLn.FindSet();
                            repeat
                                PurchLn.TestField(Quantity);
                                PurchLn.TestField("Location Code");
                                BardanaAdjmtMr.Reset();
                                BardanaAdjmtMr.SetCurrentKey("Location Code", "Arrival Seed In kg", "Adjusted Bardana in Kg");
                                BardanaAdjmtMr.SetRange("Location Code", Rec."Location Code");
                                if BardanaAdjmtMr.FindFirst() then begin
                                    PurchLn.Bardana := (PurchLn.Quantity / BardanaAdjmtMr."Arrival Seed In kg") * BardanaAdjmtMr."Adjusted Bardana in Kg";
                                    PurchLn.Modify();
                                end else
                                    Error('There is no bardana master available to calculate the bardana adjustment for this location %1', Rec."Location Code");
                            until PurchLn.Next() = 0;
                            Message('Bardana has been calculated in the arrival lines');
                        end else
                            Error('There is no Arrival line exist in the document');
                    end;
                }
            }
            group("F&unctions")
